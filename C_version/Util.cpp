...
int IpcMsgOpt::setMsggid(const char *filePathName)
{
  key_t kid;
  if (access(filePathName, F_OK) == -1)
  {
    int newfd = creat(filePathName, O_CREAT | O_RDONLY);
    close(newfd);
  }
  if((kid = ftok(filePathName, 1)) == -1)
    return -1;
  if((msggid = msgget(kid,IPC_CREAT|0666)) < 0)
  {
    fprintf(stderr, "%s:%d creat message queue fail\n", __FILE__, __LINE__);
    return -2;
  }  
  return 0;
}

void IpcMsgOpt::setNoWait(int nowait)
{
  isNowait = nowait;
}

int IpcMsgOpt::sendMsg(const void *sndBuff,int buffLen)
{
  int ret = 0;
  if(buffLen > maxLen)
    return -3;
  msgBuff.msgtype=1;
  memcpy(msgBuff.msgContent,sndBuff,buffLen);
  while ((ret = msgsnd(msggid,(void *)&msgBuff,buffLen,isNowait)) < 0)
  {
...
int IpcMsgOpt::recvMsg(void *rcvBuff,int buffLen)
{
  int ret = 0;
  if(buffLen > maxLen)
    return -1;
  MsgBuff _rcvBuff;
  while ((ret = msgrcv(msggid,(void *)&_rcvBuff,buffLen,0,0)) < 0)
  {
    if (errno == EINTR|| errno == EAGAIN)
    {
      usleep(1000);
  ...
  memcpy(rcvBuff,_rcvBuff.msgContent,buffLen);
  return 0;
}
